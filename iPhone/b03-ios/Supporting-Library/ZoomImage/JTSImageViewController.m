...
- (void)viewDidLoad {
    ...
    [button setTitleColor:[UIColor colorWithRed:104.0/255.0 green:10.0/255.0 blue:226.0/255.0 alpha:1.0] forState:UIControlStateNormal];
    [button.titleLabel setFont:[UIFont fontWithName:@"Hanken-Book" size:28.0]];
    button.frame = CGRectMake(self.view.bounds.size.width-65, 20, 60.0, 60.0);
    [self.view addSubview:button];
}


-(void) editImageAction:(UIButton*)sender
{
    [self dismiss:YES];
    [self performSelector:@selector(cropTapped) withObject:nil afterDelay:0.5];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([_userDetail valueForKey:@"name"]) {
        
        UIView * infoView = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - 100, [UIScreen mainScreen].bounds.size.width, 100.0f)];
        infoView.backgroundColor = [UIColor whiteColor];
        [self.view addSubview:infoView];
        
        UILabel *nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(20,20, [UIScreen mainScreen].bounds.size.width-100, 20)];
        [nameLabel setFont:[UIFont systemFontOfSize:16]];
        nameLabel.backgroundColor = [UIColor clearColor];
        nameLabel.textColor = [UIColor blackColor];
        nameLabel.textAlignment = NSTextAlignmentLeft;
        nameLabel.text = [NSString stringWithFormat:@"%@",[_userDetail valueForKey:@"name"]] ;
        
        UILabel *leaugeLabel = [[UILabel alloc] initWithFrame:CGRectMake(20,nameLabel.frame.origin.y + nameLabel.frame.size.height, [UIScreen mainScreen].bounds.size.width-100, 20)];
        [leaugeLabel setFont:[UIFont systemFontOfSize:14]];
        leaugeLabel.backgroundColor = [UIColor clearColor];
        leaugeLabel.textColor = [UIColor blackColor];
        leaugeLabel.textAlignment = NSTextAlignmentLeft;
        leaugeLabel.text = [NSString stringWithFormat:@"%@ League",[_userDetail valueForKey:@"level"]] ;
        
        UILabel *crownLabel = [[UILabel alloc] initWithFrame:CGRectMake(20,leaugeLabel.frame.origin.y + leaugeLabel.frame.size.height, [UIScreen mainScreen].bounds.size.width-100, 20)];
        [crownLabel setFont:[UIFont systemFontOfSize:14]];
        crownLabel.backgroundColor = [UIColor clearColor];
        crownLabel.textColor = [UIColor blackColor];
        crownLabel.textAlignment = NSTextAlignmentLeft;
        crownLabel.text = [NSString stringWithFormat:@"Lifetime Crowns: %@",[_userDetail valueForKey:@"crown"]] ;
        
        UIImageView *crownImage =[[UIImageView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 50,40,30,20)];
        crownImage.contentMode = UIViewContentModeScaleAspectFit;
        crownImage.image = [self imageForCrown:[_userDetail valueForKey:@"level"]];
        
        [infoView addSubview:nameLabel];
        [infoView addSubview:leaugeLabel];
        [infoView addSubview:crownLabel];
        [infoView addSubview:crownImage];
    }
    
}

- (UIImage *)imageForCrown:(NSString *)level{
    if ([level isEqualToString:@"Silver"]) {
        return [UIImage imageNamed:@"silverSmallFilled"];
    } else if ([level isEqualToString:@"Gold"]) {
        return [UIImage imageNamed:@"goldSmallFilled"];
    } else if ([level isEqualToString:@"Purple"]) {
        return [UIImage imageNamed:@"purpleCrown.png"];
    }else if ([level isEqualToString:@"Rose Gold"]) {
        return [UIImage imageNamed:@"roseGoldSmallFilled"];
    }else {
        return [UIImage imageNamed:@"diamondSmallFilled"];
    }
}
...
- (void)viewDidLoadForImageMode {
    ...
    self.blackBackdrop.backgroundColor = [UIColor whiteColor];//[UIColor colorWithRed:234.0/255 green:234.0/255 blue:234.0/255 alpha:1];
    self.blackBackdrop.alpha = 1;
    ...
    if([[NSUserDefaults standardUserDefaults] boolForKey:@"showButtons"]==YES)
    {
        [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"showButtons"];
        
        editBtn=[UIButton buttonWithType:UIButtonTypeCustom];
        editBtn.frame=CGRectMake(self.view.frame.size.width-100, self.view.frame.size.height-50, 100, 50);
        [editBtn addTarget:self action:@selector(editImageAction:)forControlEvents:UIControlEventTouchUpInside];
        [editBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [editBtn setTitle:@"Update" forState:UIControlStateNormal];
        [self.view addSubview:editBtn];
        
        CancelBtn=[UIButton buttonWithType:UIButtonTypeCustom];
        CancelBtn.frame=CGRectMake(10, self.view.frame.size.height-50, 100, 50);
        [CancelBtn addTarget:self action:@selector(CancelAction:)forControlEvents:UIControlEventTouchUpInside];
        [CancelBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [CancelBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:24.0]];
...
- (void)viewDidLoadForAltTextMode {
    ...
    self.blackBackdrop = [[UIView alloc] initWithFrame:CGRectInset(self.view.bounds, -512, -512)];
    self.blackBackdrop.backgroundColor = [UIColor colorWithRed:234.0/255 green:234.0/255 blue:234.0/255 alpha:1];
    self.blackBackdrop.alpha = 1;
    [self.view addSubview:self.blackBackdrop];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                weakSelf.blackBackdrop.alpha = 1;
