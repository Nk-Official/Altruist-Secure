...
class PhotoEditorViewController: UIViewController {
    ...
    fileprivate var lastEditChangeImage: MTIImage?
    ...
    @objc func saveBarButtonTapped(_ sender: Any) {
        ...
        outputImage?( Media(image: uiImage) )
//        saveImageToCameraRoll(uiImage: uiImage)
    }
    
    func saveImageToCameraRoll(uiImage: UIImage){
        PHPhotoLibrary.shared().performChanges({
            let rqst = PHAssetCreationRequest.creationRequestForAsset(from: uiImage)
        }) { (success, error) in
            if success{
    ...
    fileprivate func setupToolDataSource() {
        ...
        allTools.append(FilterToolItem(type: .adjust, slider: .zeroToHundred))
...
extension PhotoEditorViewController: FilterControlViewDelegate {
    ...
    func filterControlViewDidPressCancel() {
        imageView.image = lastEditChangeImage
        dismissFilterControlView()
    }
    
    func filterControlViewDidPressDone() {
        lastEditChangeImage = imageView.image
        dismissFilterControlView()
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == filterCollectionView {
            if currentSelectFilterIndex == indexPath.item {
                ...
                filter.inputImage = originInputImage
                imageView.image = filter.outputImage
                lastEditChangeImage = filter.outputImage
