...
extension Notification {
    func keyboardDuration() -> Double {
        if let value = self.userInfo?[UIKeyboardAnimationDurationUserInfoKey] as? NSNumber {
    ...
    func keyboardEndFrame() -> CGRect {
        if let value = self.userInfo?[UIKeyboardFrameEndUserInfoKey] as? NSValue {
            return value.cgRectValue
        } else {
            return CGRect.zero // default
        }
    }
}
...
extension UIView {
    ...
    func removeInnerShadow() {
        for view in self.subviews where view.tag == 2639 {
    ...
    func addBorder(_ edges: UIRectEdge, colour: UIColor = .white, thickness: CGFloat = 1) -> [UIView] {
